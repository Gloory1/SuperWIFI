#!/bin/sh
# =============================================================================
# SuperWiFi OTA Update Script — v5.6.2
# =============================================================================
# Changes in this release:
#   [FIX] CSS flexbox 'gap' replaced with margins → fixes layout on Android 9
#   [FIX] nftables DNS redirect (port 53) → fixes captive portal on custom DNS
#   [FIX] navigator.clipboard safe fallback → fixes JS error on HTTP context
#   [FIX] SQLite time_block UTC→localtime → fixes time-block accuracy
#   [FIX] DB migration auto-applied (v_acct_target + v_restore views recreated)
# =============================================================================
...
CURRENT_VER="unknown"
[ -f /etc/superwifi-version ] && CURRENT_VER=$(cat /etc/superwifi-version)
...
SRC_DIR="$WORK_DIR/superwifi-bash-v5/src"
[ -d "$SRC_DIR" ] || die "Expected src/ dir not found in tarball"
...
if [ -f "$DB_PATH" ]; then
    ...
    if [ "$DB_VER" != "5.6.2" ]; then
        log_warn "Applying migration v5.6.2: recreating time_block views (UTC→localtime fix)..."
        sqlite3 "$DB_PATH" "DROP VIEW IF EXISTS v_acct_target;" 2>/dev/null || true
        sqlite3 "$DB_PATH" "DROP VIEW IF EXISTS v_restore;"     2>/dev/null || true
        sqlite3 "$DB_PATH" ".read /etc/superwifi/schema.sql"    2>/dev/null || true
        sqlite3 "$DB_PATH" \
            "UPDATE system_config SET value='5.6.2', updated_at=strftime('%s','now') WHERE key='firmware_version';" \
            2>/dev/null || true
        log_info "Migration v5.6.2 applied (v_acct_target + v_restore recreated)"
    else
        log_info "DB already at v5.6.2 — migration skipped"
    fi
else
...
/etc/init.d/superwifi running 2>/dev/null && \
    ...
    log_warn "Service status check skipped (init script may not support 'running')"
...
printf "${GRN}║  Database preserved and migrated         ║${RST}\n"
printf "${GRN}║  Android 9 layout fix applied            ║${RST}\n"
printf "${GRN}║  DNS redirect (captive portal) fixed     ║${RST}\n"
printf "${GRN}║  Time blocks now use local timezone      ║${RST}\n"
printf "${GRN}╚══════════════════════════════════════════╝${RST}\n\n"
