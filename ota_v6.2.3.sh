#!/bin/sh
# =============================================================================
# SuperWiFi OTA Update Script — v6.2.3
# =============================================================================
# Changes in this release:
#   [ARCH] Decoupled cloud sync from sw-acct daemon into OpenWrt Cron (/etc/crontabs/root)
#   [FEAT] Dynamic sync interval reading from UCI config option sync_interval (default 5m)
#   [FIX]  Automatic crontab management & cron service activation in init script
#   [FIX]  Bulk package deletion matching by both ID and Name to prevent stale packages
# =============================================================================
...
# =============================================================================
# 4. Apply DB migration (v6.2.3 SSOT & CLEANUP)
# =============================================================================
...
if [ -f "$DB_PATH" ]; then
    ...
    if [ "$DB_VER" != "$VERSION" ]; then
        log_warn "Applying migration v${VERSION}: updating schema..."
...
printf "${GRN}║  Decoupled Sync    OpenWrt Crontab       ║${RST}\n"
printf "${GRN}║  Sync Interval     UCI option (5m)       ║${RST}\n"
printf "${GRN}╚══════════════════════════════════════════╝${RST}\n\n"
