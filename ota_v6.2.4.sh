#!/bin/sh
# =============================================================================
# SuperWiFi OTA Update Script — v6.2.4
# =============================================================================
# Changes in this release:
#   [FIX]  Guaranteed class_id allocation in db_reauth_session for reconnected users
#   [FIX]  Traffic accounting now counts restored & reconnected sessions flawlessly
#   [FIX]  Automatic class_id gap-finding & season_bytes reset in init script restore
#   [FIX]  Instant cloud visibility & sync for reconnected & restored vouchers
# =============================================================================
...
# =============================================================================
# 4. Apply DB migration (v6.2.4 SSOT & RECOVERY)
# =============================================================================
log_step "4/5 · Database migration & recovery"
...
if [ -f "$DB_PATH" ]; then
    ...
    log_warn "Applying migration v${VERSION}: repairing active session class_id mapping..."
    sqlite3 "$DB_PATH" ".read /etc/superwifi/schema.sql" 2>/dev/null || true
    sqlite3 "$DB_PATH" "
        BEGIN EXCLUSIVE;
        UPDATE session SET season_usage_bytes = 0 WHERE status = 'active';
