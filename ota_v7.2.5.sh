...
if command -v curl >/dev/null 2>&1; then
    curl -fsSL "$GITHUB_URL" -o "${WORK_DIR}/${TARBALL}" || die "curl download failed"
elif command -v wget >/dev/null 2>&1; then
    wget -q "$GITHUB_URL" -O "${WORK_DIR}/${TARBALL}" || die "wget download failed"
else
