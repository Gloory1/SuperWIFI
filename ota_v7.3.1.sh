#!/bin/sh
# =============================================================================
# SuperWiFi OTA Update Script — v7.3.1
# =============================================================================
# Changes in this release:
#   [FEATURE] Added Lazy & Eager Session Restore strategies (configurable via UCI
#             superwifi.core.restore_mode='lazy'|'eager', defaulting to 'lazy').
#   [FEATURE] Integrated dnsmasq dhcp-script hook (/usr/lib/superwifi/dhcp-restore-hook.sh)
#             for on-demand lazy session restoration upon DHCP lease events.
#   [OPTIMIZE] Included s.id in v_restore view and added db_get_restore_session_by_mac.
#   [FIX] Multi-tool TLS/SSL download handling (curl, wget --no-check-certificate, uclient-fetch).
# =============================================================================
...
[ "$DL_OK" = "1" ] && [ -s "${WORK_DIR}/${TARBALL}" ] || die "Failed to download $TARBALL from $GITHUB_URL (network or SSL error)"
...
# =============================================================================
# 3. Install files (preserving /etc/config/superwifi)
# =============================================================================
...
# Daemon scripts
[ -f "$SRC_DIR/usr/sbin/sw-acct.sh" ] && cp "$SRC_DIR/usr/sbin/sw-acct.sh" /usr/sbin/
[ -f "$SRC_DIR/usr/sbin/sw-sync.sh" ] && cp "$SRC_DIR/usr/sbin/sw-sync.sh" /usr/sbin/
[ -f "$SRC_DIR/usr/sbin/sw"         ] && cp "$SRC_DIR/usr/sbin/sw"          /usr/sbin/
...
if [ -f "$DB_PATH" ]; then
    ...
    # Source the new core scripts to use the fixed db_repair_active_sessions_class_id function
...
# Ensure UCI restore_mode is explicitly set to 'lazy' if missing
if ! uci get superwifi.core.restore_mode >/dev/null 2>&1; then
    uci set superwifi.core.restore_mode='lazy'
    uci commit superwifi
    log_info "Configured superwifi.core.restore_mode='lazy'"
fi

# =============================================================================
# 5. Start service & Configure Cron
# =============================================================================
log_step "5/5 · Starting service & crontab"
...
printf "${GRN}╠══════════════════════════════════════════╣${RST}\n"
printf "${GRN}║  Accounting Bug    Fixed (ClassID gap)   ║${RST}\n"
