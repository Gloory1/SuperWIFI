#!/bin/sh
# =============================================================================
# SuperWiFi OTA Update Script — v8.1.3
# =============================================================================
# Changes in this release:
#   [FIX] Repair & unique class_id assignment for all existing/legacy vouchers.
#   [FIX] Rebuild kernel TC classes & filters in proper order during startup/repair.
#   [ENGINE] Unified sw-engine.sh daemon replacing standalone loops.
#   [UPDATE] Optional SHA256 verification in 'sw update' for seamless DB updates.
#   [MIGRATION] Automatic migration & initialization of acct_interval in UCI.
#   [CLEANUP] Purged legacy paths (/usr/lib/superwifi/sw-sentinel.sh, etc.).
...
#   [SYSTEM] Automatic router reboot after update completion.
# =============================================================================
...
if [ -f "$DB_PATH" ]; then
    ...
    log_info "Repairing missing/duplicate class_ids & rebuilding TC filters..."
...
printf "${GRN}║  Session Class IDs Repaired & Restored   ║${RST}\n"
printf "${GRN}║  SSOT versioning   v8.1.3 Release        ║${RST}\n"
printf "${GRN}╚══════════════════════════════════════════╝${RST}\n\n"
